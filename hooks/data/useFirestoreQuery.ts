...
// --- Types ---
type TFirestoreStatus = 'idle' | 'loading' | 'success' | 'error'

interface IFirestoreState<T> {
  status: TFirestoreStatus
  data?: T | T[] | null
  error?: Error
}

type TFirestoreAction<T> =
  | { type: 'idle' }
  | { type: 'loading' }
...
// --- Reducer ---
function reducer<T>(
  _state: IFirestoreState<T>,
  action: TFirestoreAction<T>,
): IFirestoreState<T> {
  switch (action.type) {
    case 'idle':
...
function useFirestoreQuery<T = DocumentData>(
  ...
  const initialState: IFirestoreState<T | T[]> = {
